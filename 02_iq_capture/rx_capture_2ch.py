...
class DualRxCapture(gr.top_block):
    def __init__(self, args: str, freq: float, rate: float, gain: float, antenna: str, seconds: float, out_dir: Path, probe_rate: float, tx_scale: float, pilot_repeats_per_tx: int, frame_format: str, sync_tx_mode: str, tx_chain_mode: str, active_carrier_count: int, tx1_cyclic_shift_samples: int):
        ...
        cfg = {"uhd_args": args, "center_freq": freq, "sample_rate": rate, "rx_gain_db": gain,
               ...
               "dtype": "complex64", "frame_format": frame_format, "sync_tx_mode": sync_tx_mode,
               "tx_chain_mode": tx_chain_mode, "active_carrier_count": active_carrier_count,
        ...
        probe_cfg = ProbeConfig(sample_rate=rate, center_freq=freq, fft_len=CFG.fft_len, cp_len=CFG.cp_len,
                                ...
                                frame_format=frame_format, sync_tx_mode=sync_tx_mode,
                                tx_chain_mode=tx_chain_mode,
                                tx1_cyclic_shift_samples=tx1_cyclic_shift_samples,
                                seed=CFG.seed)
        save_probe_metadata(out_dir / "probe_metadata.json", probe_cfg)
        print(f"RX: {freq/1e6:.6f} MHz, {rate/1e6:.3f} MS/s, gain={gain:.1f} dB, duration={seconds:.1f}s, active_carriers={active_carrier_count}, format={frame_format}, sync_tx_mode={sync_tx_mode}, tx_chain_mode={tx_chain_mode}, tx1_csd={tx1_cyclic_shift_samples} samples, out={out_dir}")
...
def main():
    a = parse_args(); tb = DualRxCapture(a.args, a.freq, a.rate, a.gain, a.antenna, a.seconds, a.out_dir, a.probe_rate, a.tx_scale, a.pilot_repeats_per_tx, a.frame_format, a.sync_tx_mode, a.tx_chain_mode, a.active_carrier_count, a.tx1_cyclic_shift_samples)
