...
class MimoProbeTx(gr.top_block):
    def __init__(self, args: str, freq: float, rate: float, gain: float, antenna: str,
        ...
        print(f"TX: {freq/1e6:.6f} MHz, {rate/1e6:.3f} MS/s, probe={probe_rate:.1f} Hz, gain={gain:.1f} dB, frame={meta['frame_len']} samples, format={meta['frame_format']}, sync_tx_mode={meta.get('sync_tx_mode', 'both')}, tx_chain_mode={meta.get('tx_chain_mode', 'both')}")
...
def main():
    a = parse_args()
    tb = MimoProbeTx(a.args, a.freq, a.rate, a.gain, a.antenna, a.probe_rate, a.tx_scale, a.pilot_repeats_per_tx, a.frame_format, a.sync_tx_mode, a.tx_chain_mode)
    def stop_handler(*_):
